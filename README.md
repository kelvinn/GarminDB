...
GarminDB can also store data in Postgres. Set `"db": {"type": "postgres", "db_username": "garmindb", "db_password": "garmindb", "db_host": "localhost", "db_port": 5432, "db_name": "garmindb"}` in GarminConnectConfig.json. Postgres support uses one database with one schema per GarminDB logical database and sets schema search path at transaction start for compatibility with transaction poolers. Optional Postgres resilience settings are available in `db`: `postgres_connect_timeout_sec`, `postgres_statement_timeout_ms`, `postgres_retry_attempts`, `postgres_retry_base_backoff_sec`, and `postgres_retry_max_backoff_sec`. Install the Postgres driver with `pip install 'psycopg[binary]'`.
